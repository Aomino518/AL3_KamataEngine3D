...
void DethParticles::Update() {
	...
	for (uint32_t i = 0; i < 8; ++i) {
		...
		// Z軸まわりの回転行列
		Matrix4x4 matrixZRotation = MakeRotateZMatrix(angle);
		Matrix4x4 matrixXRotation = MakeRotateYMatrix(angle);
		Matrix4x4 matrixXZRotation = Multiply(matrixXRotation, matrixZRotation);
		// 基本ベクトル回転させて速度ベクトルを得る
		velocity = Transform(velocity, matrixXZRotation);
		// 移動処理
